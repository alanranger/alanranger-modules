...
function rowDistanceScore(rowChips) {
  const total = rowChips.length;
  const hasOrderIndex = rowChips.every((chip) =>
    Number.isFinite(chip?.orderIndex)
  );
  if (!hasOrderIndex || total <= 1) return null;
  let distance = 0;
  let maxDistance = 0;
  for (let i = 0; i < total; i += 1) {
    const expectedIndex = rowChips[i].orderIndex;
    distance += Math.abs(expectedIndex - i);
    maxDistance += Math.abs(i - (total - 1 - i));
  }
  if (!maxDistance) return 100;
  const penalty = Math.min(1, distance / maxDistance);
...
export function scoreHueTest(rows, bands) {
  ...
  rows.forEach((row) => {
    ...
    const distanceScore = rowDistanceScore(row);
    totalScore += Number.isFinite(distanceScore) ? distanceScore : accuracy;
    totalCount += 1;
  ...
  return {
    totalScore: Math.round(totalScore / Math.max(1, totalCount)),
    rowScores,
    bandErrors
  };
}
