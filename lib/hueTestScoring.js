...
function scoreRow(rowChips) {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const isCorrect = hasOrderIndex && rowChips[i].orderIndex === i;
    score += error;
    chipErrors.push({
      id: rowChips[i].id,
      hue: actual,
...
export function scoreHueTest(rows, bands) {
  ...
  const bandCorrectCounts = new Array(bands.length).fill(0);
  const rowScores = [];
  let totalScore = 0;
  const hasOrderIndex = rows.every((row) =>
    row.every((chip) => Number.isFinite(chip?.orderIndex))
  );
  ...
  const bandAccuracy = hasOrderIndex
    ? bandCorrectCounts.map((count, idx) => {
        const total = bandCounts[idx];
        if (!total) return 0;
