...
const { DELIVERED_STATUSES } = require("./emailEvents");
const {
...
function isPaidAudience(snapshot) {
  return snapshot.isPaid && !snapshot.isActiveTrial;
}

function isBadgeAudience(snapshot) {
  return Boolean(snapshot?.isActiveTrial || isPaidAudience(snapshot));
}

async function evaluateBadgeCrossingTrigger(supabase, memberId, snapshot) {
  if (!isBadgeAudience(snapshot)) return false;
  ...
  return Boolean(findBadgeCrossing(ctx.badges, celebratedRank));
}

function evaluateRenewalSoonTrigger(snapshot, trigger, nowMs = Date.now()) {
  if (!isPaidAudience(snapshot)) return false;
  const plan = snapshot.planSummary || {};
  const renewal = parseRenewalContext(plan, nowMs);
  if (!renewal) return false;
  const targetDays = trigger.renewalWithinDays ?? 14;
  return renewal.daysUntilRenewal === targetDays;
}

async function evaluateStageTrigger(supabase, snapshot, stageKey, memberId, nowMs = Date.now()) {
  if (!snapshot) return false;
  const stage = getStageByKey(stageKey);
  if (!stage || !stage.trigger) return false;
  if (stage.deprecated || stage.trigger.deprecated) return false;

  const t = stage.trigger;
  if (t.audience === "paid" && !isPaidAudience(snapshot)) return false;
  if (t.audience === "paid_or_active_trial" && !isBadgeAudience(snapshot)) return false;
  if (t.audience !== "paid" && t.audience !== "paid_or_active_trial" && snapshot.hasConverted) return false;
  ...
  if (t.badgeCrossing) {
    return evaluateBadgeCrossingTrigger(supabase, memberId, snapshot);
  }

  if (t.renewalWithinDays != null) {
    return evaluateRenewalSoonTrigger(snapshot, t, nowMs);
  }

  if (t.eventDriven) return false;
  ...
  if (snapshot.reengagementSendCount >= 4) return false;
...
async function fetchCandidateMemberIds(supabase, stageKey) {
  const stage = getStageByKey(stageKey);
  if (!stage || !stage.trigger || stage.deprecated) return [];
  const t = stage.trigger;
...
async function countEligibleForStage(supabase, stageKey, contactable, nowMs = Date.now()) {
  ...
  if (stageKey === "paid-badge-earned") {
    const rows = await listPaidBadgeEligible(supabase, contactable, nowMs);
    return rows.length;
  }
  if (stageKey === "paid-renewal-soon") {
    const rows = await listRenewalSoonEligible(supabase, contactable, nowMs);
    return rows.length;
  }
  ...
  for (const memberId of candidates) {
    ...
    if (!snapshot || !(await evaluateStageTrigger(supabase, snapshot, stageKey, memberId, nowMs))) continue;
