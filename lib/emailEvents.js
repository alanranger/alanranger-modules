...
const SEND_SOURCES = Object.freeze(["automated", "manual_batch", "corrected_resend", "test"]);

const MANUAL_SEND_SOURCES = Object.freeze(["manual_batch", "corrected_resend"]);

/** SMTP 250 accepted is enough; gmail_verified is optional IMAP bonus. */
const DELIVERED_STATUSES = Object.freeze(["smtp_accepted", "gmail_verified"]);

function isDeliveredStatus(value) {
  return DELIVERED_STATUSES.includes(String(value || ""));
}
...
const STAGE_KEYS = [
  ...
  "paid-milestone",
  "paid-renewal-soon",
  // One-off correction email (broken upgrade CTA) — not a cron ladder rung.
  "cta-fix",
];
...
async function logEmailEvent(supabase, opts) {
  ...
  const deliveryStatus =
    opts.deliveryStatus
    || (status === "sent" ? "smtp_accepted" : null);
  const row = {
    ...
    dry_run: !!opts.dryRun,
    event_detail: opts.eventDetail || null,
    send_source: SEND_SOURCES.includes(opts.sendSource) ? opts.sendSource : "automated",
