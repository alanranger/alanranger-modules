 ...
 *
 * Delivery truth = SMTP-accepted (or gmail_verified) rows in academy_email_events.
 * Do not compare Gmail Sent To: counts — IMAP Sent is unreliable for this mailbox.
 */
...
function classifyWatchdogRow({
  ...
  if (loggedSent > 0) {
    return {
      state: "verified",
      label: "✅ Verified",
      detail: "SMTP-accepted sends in last 24h",
    };
  }
...
function watchdogHtml(report, banner) {
  ...
  return [
    ...
    `<p style="font-size:12px;color:#5f5f5f;margin:0 0 14px">${escapeHtml(report.note || report.gmailNote || "")}</p>`,
    '<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%">',
    "<thead><tr>",
    `<th style="${HEAD}">Stage</th>`,
    `<th style="${HEAD};text-align:right">Eligible today</th>`,
    `<th style="${HEAD};text-align:right">SMTP-accepted log (24h)</th>`,
    ...
    "Catch-up / corrected batch sends are excluded from the 24h column.",
    "</p>",
    "</div>",
  ].join("");
}
...
function formatWatchdogEmail(report) {
  ...
  const note = report.note || report.gmailNote || "";
  const lines = [
    banner,
    "",
    ...
    note,
    "",
    "Stage | Eligible today | SMTP-accepted log (24h) | State",
    "----- | -------------- | ----------------------- | -----",
  ];
  for (const row of report.rows) {
    lines.push(
      `${row.label} | ${eligibleCellFor(row)} | ${row.loggedSent} | ${row.firingLabel}`
    );
