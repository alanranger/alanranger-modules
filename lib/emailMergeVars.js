...
function badgeGapPhrase(modules, exams, modulesOpened) {
  const opened = Number(modulesOpened);
  if (Number.isFinite(opened) && opened === 0) return "still near the start";
...
function enrichRenderVars(vars) {
  ...
  base.badgeGapPhrase = badgeGapPhrase(
    base.modulesToNextBadge,
    base.examsToNextBadge,
    base.modulesOpened
  );
