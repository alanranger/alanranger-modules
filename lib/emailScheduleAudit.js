...
/** Per-stage cron overrides (expensive stages get their own path). */
const STAGE_REQUIRED_CRON_PATHS = {
  "paid-badge-earned": [
    "/api/admin/triggered-email-webhook?stageKey=paid-badge-earned&sendEmail=true",
  ],
};
...
function missingCronPaths(vercelJson) {
  ...
  const missing = [];
  const allRequired = [
    ...Object.values(REQUIRED_CRON_PATHS).flat(),
    ...Object.values(STAGE_REQUIRED_CRON_PATHS).flat(),
  ];
...
function auditStageSchedule(stage, vercelJson) {
  ...
  const required =
    STAGE_REQUIRED_CRON_PATHS[stage.key] || REQUIRED_CRON_PATHS[stage.sentBy] || [];
  const configured = new Set(cronPathsFromVercelJson(vercelJson));
