...
function computeNextBadgeProgress(badges, stats, activeDays, engagementDegraded, openedCount, modulesTotal) {
  ...
  if (!next) {
    return { pct: 100, label: "100% - all badges earned", degraded: false, nextKey: null };
  }

  if ((next.key === "graduate" || next.key === "master") && isLongevityDegraded(stats)) {
    const pct = Math.round((safeNum(openedCount, 0) / modTotal) * 100);
    return { pct, label: `${pct}% of the foundations course`, degraded: true, nextKey: next.key };
  }

  const ratios = [];
  if (next.key === "foundation") {
    ratios.push(componentRatio(safe.foundationModulesOpened, FOUNDATION_GATE.minModules));
    if (!engagementDegraded) {
      ratios.push(componentRatio(safeNum(activeDays, 0), FOUNDATION_GATE.minActiveDays));
    }
  ...
  return { pct, label: `${pct}% to ${next.label}`, degraded: false, nextKey: next.key };
}
