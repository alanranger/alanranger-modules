...
/** Visual target lines on tiles (provisional, not computed). Set as reachable next-step milestones above current baselines — revisit as sample grows. */
...
const COHORT_DEFINITION = {
  ...
  exclude_sources: ["academy_plan_events_backfill"],
  exclude_emails: [...INTERNAL_EMAILS],
  exclude_member_id_prefix: "mem_qa_",
  description:
    "Organic trials only (source=stripe_webhook), excluding internal emails and backfill rows. " +
    "Metrics are per-member from each trial start date, then aggregated for signups in the selected period.",
};
...
async function buildActivationTargetsStats(supabase, period) {
  ...
  return {
    ...
    cohort_definition: COHORT_DEFINITION.description,
    period,
