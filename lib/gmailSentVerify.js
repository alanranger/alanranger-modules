/**
 * Read Gmail Sent for Academy lifecycle mail (same mailbox as SMTP).
 * Uses IMAP with ORPHANED_EMAIL_* app password — no extra OAuth.
 *
 * Message-ID match is the delivery truth. Nodemailer invents messageId
 * locally; only a Message-ID present in Sent proves the message left.
 */
...
async function openSentMailbox(client) {
  try {
...
async function confirmMessageIdInSent(messageId, opts = {}) {
  ...
  for (let i = 0; i < attempts; i += 1) {
    last = await messageIdInGmailSent(messageId);
