...
function findFirstUnpassedExam(statusMap) {
  for (let i = 0; i < EXAM_MODULE_IDS.length; i += 1) {
    const id = EXAM_MODULE_IDS[i];
    if (statusMap[id] !== "passed") {
      return { idx: i, id, track: "foundation" };
    }
  }
  for (let i = 0; i < COMPOSITION_EXAM_MODULE_IDS.length; i += 1) {
    const id = COMPOSITION_EXAM_MODULE_IDS[i];
    if (statusMap[id] !== "passed") {
      return { idx: i, id, track: "composition_creative" };
    }
  }
  return null;
}
...
function parseExamProgress(progressData) {
  ...
  if (source === "modules-count" || source === "tracks-modules") {
    const nextExam = findFirstUnpassedExam(statusMap);
    nextExamModuleId = nextExam ? nextExam.id : null;
  }
