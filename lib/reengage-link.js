...
function buildPersonalUpgradeUrl(memberId, email, windowExpiresAtMs) {
  ...
  // Original REWIND win-back pattern (6dadcce → pre-e6e818b): land on the dashboard
  // so the snippet can stash ?ar_rewind= in sessionStorage before any login bounce.
  // Trial SAVE20 emails use reengage-redirect; win-back never did — login page does
  // not consume ?ar_rewind= on its own (see academy-login-rewind-prefill-snippet.html).
