...
/** PostgREST defaults to 1000 rows; paginate like api/admin/engagement.js fetchEvents. */
...
/** Latest send per member+stage since a window start (for 90-day outcomes). */
...
async function buildEmailEngagementStats(supabase) {
  ...
  const sends = await fetchEmailSends(supabase, sinceFetch);
  ...
  return {
    ...
    note:
      "Outcomes use latest send per member per stage in the window. Login/module after = academy_events strictly after sent_at (all pages fetched). Emailed (90d) = deduped count; Raw sends = every logged row from cron/webhooks.",
    debug: {
