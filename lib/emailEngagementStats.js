...
async function buildEmailEngagementStats(supabase) {
  ...
  const [sends, events, conversions, loggingMeta] = await Promise.all([
    fetchEmailSends(supabase, sinceFetch),
    fetchEventsSince(supabase, sinceFetch),
  ...
  return {
    ...
    note:
      "Outcomes use latest send per member per stage in the window. Emailed (90d) = deduped count; Raw sends = every logged row from cron/webhooks.",
  };
}
