...
async function listPaidBadgeEligible(supabase, contactable, nowMs = Date.now()) {
  // Only paid members, parallel scans, soft deadline — avoids Vercel 300s kill.
  const paidIds = await listPaidContactableIds(supabase, contactable, nowMs);
  const celebratedByMember = await loadCelebratedBadgeRanks(supabase, paidIds);
  const out = [];
  ...
  async function worker() {
    while (cursor < paidIds.length) {
      if (Date.now() > deadlineMs) return;
      const memberId = paidIds[cursor++];
  ...
  const workers = Math.min(concurrency, Math.max(paidIds.length, 1));
