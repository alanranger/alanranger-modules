...
function buildPractitionerLines(g) {
  ...
  if (examNeed > 0) {
    lines.push(`- Pass ${examNeed} more exam${examNeed === 1 ? "" : "s"} (${g.examsPassed} of ${g.examsPassedTarget} needed)`);
  }
  return lines;
}
...
function buildCertifiedLines(g) {
  const lines = [];
  const examNeed = Math.max(0, 15 - (g.examsPassed || 0));
  const modNeed = Math.max(0, (g.totalModulesTarget || 0) - (g.totalModulesOpened || 0));
  if (examNeed > 0) lines.push(`- Pass ${examNeed} more exam${examNeed === 1 ? "" : "s"} (${g.examsPassed} of 15 done)`);
  if (modNeed > 0) {
    lines.push(
      `- Open ${modNeed} more foundation module${modNeed === 1 ? "" : "s"} (${g.totalModulesOpened} of ${g.totalModulesTarget} done)`
    );
  }
  return lines;
}
...
async function fetchExamsPassed(supabase, memberId, email) {
  const { data } = await supabase.from("module_results_ms").select("passed").eq("memberstack_id", memberId);
...
async function buildFullBadgeContext(supabase, memberId) {
  ...
  const [examsPassed, longevity, activeWindow] = await Promise.all([
    fetchExamsPassed(supabase, memberId, memberRow.email),
    fetchLongevityFields(supabase, memberId),
    fetchFirst14ActiveDays(supabase, memberId, trialStartAt),
  ]);

  const stats = mergeEngagementIntoStats(computeGateStatsFromRaw(memberRow.raw, examsPassed), longevity);
