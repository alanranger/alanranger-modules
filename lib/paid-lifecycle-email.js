...
async function getCelebratedBadgeRank(supabase, memberId) {
  const map = await loadCelebratedBadgeRanks(supabase, [memberId]);
  return map.get(memberId) ?? -1;
}

async function loadCelebratedBadgeRanks(supabase, memberIds) {
  const map = new Map();
  ...
  for (let i = 0; i < memberIds.length; i += CHUNK) {
    ...
    (data || []).forEach((row) => {
      ...
      if (rank == null) return;
      const prev = map.get(row.member_id) ?? -1;
      if (rank > prev) map.set(row.member_id, rank);
    });
  }
  return map;
}

async function listPaidContactableIds(supabase, contactable, nowMs = Date.now()) {
  const { data } = await supabase
    .from("ms_members_cache")
  ...
  return (data || [])
    .filter((row) => {
      if (!contactable.has(row.member_id)) return false;
      const plan = row.plan_summary || {};
      return isPaidMember(plan) && !isActiveTrialPlan(plan, nowMs);
    })
    .map((row) => row.member_id);
}

/** Paid members + active trials (badge celebration audience). */
async function listBadgeContactableIds(supabase, contactable, nowMs = Date.now()) {
  const { data } = await supabase
    .from("ms_members_cache")
    .select("member_id, plan_summary")
    .not("email", "is", null);
...
async function listPaidBadgeEligible(supabase, contactable, nowMs = Date.now()) {
  // Paid + active trial, parallel scans, soft deadline — avoids Vercel 300s kill.
  const memberIds = await listBadgeContactableIds(supabase, contactable, nowMs);
  const celebratedByMember = await loadCelebratedBadgeRanks(supabase, memberIds);
  const out = [];
  const deadlineMs = nowMs + 240000;
  const concurrency = 6;
  let cursor = 0;
  ...
  async function worker() {
    while (cursor < memberIds.length) {
      ...
      if (!ctx) continue;
      if (!(ctx.snapshot.isActiveTrial || ctx.snapshot.isPaid)) continue;
      const celebratedRank = celebratedByMember.get(memberId) ?? -1;
  ...
  const workers = Math.min(concurrency, Math.max(memberIds.length, 1));
  await Promise.all(Array.from({ length: workers }, () => worker()));
  return out;
}

async function listRenewalSoonEligible(supabase, contactable, nowMs = Date.now()) {
  const paidIds = await listPaidContactableIds(supabase, contactable, nowMs);
  const out = [];
