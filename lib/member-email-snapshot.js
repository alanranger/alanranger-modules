...
const LOGIN_EVENT_TYPES = ["login", "member_login"];
...
const CERT2_MODULE_IDS = [
  "c2-01-composition-rules",
  "c2-02-framing",
  "c2-03-leading-lines",
  "c2-04-negative-space",
  "c2-05-contrast",
  "c2-06-balance",
  "c2-07-macro",
  "c2-08-landscape",
...
function findNextUnopenedModule(openedSet) {
  for (const path of FOUNDATION_MODULE_PATHS) {
    if (!openedSet.has(path)) {
      const meta = getFoundationModuleMeta(path);
      return {
        nextModuleUrl: `${SITE_BASE}${path}`,
        nextModuleTitle: meta.title,
        nextModuleRef: meta.ref,
        nextModuleSection: meta.section,
        nextModuleLabel: meta.label,
        nextModulePositionInSection: meta.positionInSection,
      };
    }
  }
  const fallback = getFoundationModuleMeta(FOUNDATION_MODULE_PATHS[0]);
  return {
    nextModuleUrl: `${SITE_BASE}${FOUNDATION_MODULE_PATHS[0]}`,
    nextModuleTitle: fallback.title,
    nextModuleRef: fallback.ref,
    nextModuleSection: fallback.section,
    nextModuleLabel: fallback.label,
    nextModulePositionInSection: fallback.positionInSection,
  };
}
...
async function fetchCert2ExamsProgress(supabase, memberId) {
  const total = CERT2_MODULE_IDS.length;
  const { data, error } = await supabase
    .from("module_results_ms")
    .select("module_id, passed, attempt")
    .eq("memberstack_id", memberId)
    .in("module_id", CERT2_MODULE_IDS);
  if (error) return { passed: 0, total };
  const latest = {};
  for (const row of data || []) {
    const prev = latest[row.module_id];
    if (!prev || (row.attempt || 0) > (prev.attempt || 0)) latest[row.module_id] = row;
  }
  const passed = Object.values(latest).filter((r) => r.passed).length;
  return { passed, total };
}
...
async function fetchLastLoginAt(supabase, memberId) {
  const { data } = await supabase
    ...
    .select("created_at")
    .eq("member_id", memberId)
    .in("event_type", LOGIN_EVENT_TYPES)
...
async function buildMemberEmailSnapshot(supabase, memberId, nowMs = Date.now()) {
  ...
  const [examCounts, cert2Progress] = await Promise.all([
    fetchExamPassCountsForMember(supabase, memberId, ctx.email),
    fetchCert2ExamsProgress(supabase, memberId),
  ]);
  const openedSet = buildOpenedSet(ctx.raw);
  const stats = computeGateStatsFromRaw(
    ctx.raw,
  ...
  return {
    ...
    nextModuleRef: nextMod.nextModuleRef,
    nextModuleSection: nextMod.nextModuleSection,
    nextModuleLabel: nextMod.nextModuleLabel,
    nextModulePositionInSection: nextMod.nextModulePositionInSection,
    trialDayNumber,
    trialDaysRemaining,
    ...
    cert2ExamsPassed: cert2Progress.passed,
    cert2ExamsTotal: cert2Progress.total,
    cert2ExamsUrl: `${SITE_BASE}/academy/photography-exams-certification`,
  };
}
