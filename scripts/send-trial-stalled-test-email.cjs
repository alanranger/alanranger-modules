/**
 * Send one trial-stalled dummy test email to info@alanranger.com (Claude BUILD).
 * Usage: node scripts/send-trial-stalled-test-email.cjs
 */
...
async function main() {
  ...
  const rendered = await renderStageEmail(supabase, "trial-stalled", TRIAL_STALLED_DUMMY_PROFILE);
  if (!rendered) throw new Error("Template render failed");
  const subject = `${DAY2_TEST_SUBJECT_PREFIX.stalled} ${rendered.subject}`;
