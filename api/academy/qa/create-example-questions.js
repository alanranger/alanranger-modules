// /api/academy/qa/create-example-questions.js
// Creates example Q&A questions for a new member
// Called automatically when a new member is created (via webhook or manual trigger)
...
// The 4 example questions (same as populate script)
...
/**
 * Creates example questions for a member
 * @param {string} memberId - Memberstack member ID
 * @param {string} memberEmail - Member email (optional)
 * @param {string} memberName - Member name (optional)
 * @returns {Promise<{success: boolean, inserted: number, error?: string}>}
 */
async function createExampleQuestionsForMember(memberId, memberEmail = null, memberName = null) {
  try {
    ...
    if (existingExamples && existingExamples.length > 0) {
      return { success: true, inserted: 0, message: 'Member already has example questions' };
    }

    // Insert all 4 example questions
    ...
    return { success: true, inserted: 4 };
  } catch (error) {
    console.error('[create-example-questions] Error:', error);
    return { success: false, inserted: 0, error: error.message };
  }
}

// Export function for use in other modules
exports.createExampleQuestionsForMember = createExampleQuestionsForMember;
...
module.exports = async function handler(req, res) {
  ...
  const { member_id, member_email, member_name } = req.body;

  if (!member_id) {
    return res.status(400).json({ error: 'member_id is required' });
  }

  const result = await createExampleQuestionsForMember(member_id, member_email, member_name);
