...
const { DELIVERED_STATUSES } = require("../../lib/emailEvents");
...
const { unscheduledCronEnabledStages } = require("../../lib/emailScheduleAudit");
...
function isCronWindowEvent(row) {

  const detail = String(row.event_detail || "");

  if (/catchup_/i.test(detail)) return false;

  if (/corrected_resend/i.test(detail)) return false;

  if (row.send_source === "corrected_resend") return false;

  if (row.send_source === "manual_batch" && /catchup_/i.test(detail)) return false;
...
async function loadEventsLast24h(sinceIso) {
  ...
  const { data, error } = await supabase
    ...
    .select("stage_key, email, created_at, delivery_status, message_id, event_detail, send_source")
  ...
  return (data || []).filter(isCronWindowEvent);
...
module.exports = async function handler(req, res) {
  ...
  const scheduleGaps = unscheduledCronEnabledStages(vercelJson);

  for (const gap of scheduleGaps) {

    const watchdogStage = WATCHDOG_STAGES.find((s) => s.key === gap.key);

    const row = rows.find((r) => r.key === gap.key);

    const patch = {

      state: "not_firing",

      firingLabel: "🔴 NOT SCHEDULED",

      detail: `cronEnabled but missing vercel.json cron for ${gap.sentBy}${gap.missing?.length ? `: ${gap.missing.join(", ")}` : ""}`,

    };

    if (row) Object.assign(row, patch);

    else if (watchdogStage) {

      rows.push({

        key: gap.key,

        label: watchdogStage.label,
  ...
  const note =
    ...
    "Delivery truth = SMTP accepted (delivery_status smtp_accepted or gmail_verified). "

    + "Gmail Sent IMAP is not used for health. Catch-up/corrected batches excluded.";
